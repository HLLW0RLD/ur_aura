...
object ZodiacLibrary {
    ...
    fun zodiacFrame(width: Int, height: Int, user: UserData): Bitmap {
        ...
        // Внутренний символ (минимально: простой знак зодиака)
        ...
        drawSimpleZodiacSymbol(canvas, user.zodiacSign, centerX, centerY, min(width, height) * 0.18f, symbolPaint)
        return bmp
    }

    private fun drawSimpleZodiacSymbol(canvas: Canvas, zodiac: String, cx: Float, cy: Float, size: Float, paint: Paint) {
        // простые символы, достаточно стилизованные линии
        when (zodiac.lowercase()) {
            ...
            // ... можно расширить для всех знаков; базовый набор для видимости
            else -> {
                // fallback: круг с радиальными линиями
